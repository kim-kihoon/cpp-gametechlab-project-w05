...
namespace Graphics
{
    namespace
    {
            ...
            URenderer::FMeshResource& res)
        {
            ...
            res.LocalAABB.Min = {FLT_MAX, FLT_MAX, FLT_MAX};
            res.LocalAABB.Max = {-FLT_MAX, -FLT_MAX, -FLT_MAX};

            for (const auto& v : res.SourceVertices)
            {
                res.LocalAABB.Expand(v.Position);
            }

            res.LocalCenter = {(res.LocalAABB.Min.x + res.LocalAABB.Max.x) * 0.5f,
                               (res.LocalAABB.Min.y + res.LocalAABB.Max.y) * 0.5f,
                               (res.LocalAABB.Min.z + res.LocalAABB.Max.z) * 0.5f};

            float SizeX = res.LocalAABB.Max.x - res.LocalAABB.Min.x;
            float SizeY = res.LocalAABB.Max.y - res.LocalAABB.Min.y;
            float SizeZ = res.LocalAABB.Max.z - res.LocalAABB.Min.z;
            res.LocalRadius = (std::max)({SizeX, SizeY, SizeZ}) * 0.5f;


            D3D11_BUFFER_DESC VertexBufferDesc = {};
            VertexBufferDesc.ByteWidth = static_cast<UINT>(res.SourceVertices.size() * sizeof(URenderer::FMeshVertex));
            VertexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
            VertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
            D3D11_SUBRESOURCE_DATA VertexData = {res.SourceVertices.data(), 0, 0};
            if (FAILED(dev->CreateBuffer(&VertexBufferDesc, &VertexData, &res.VertexBuffer))) return false;
            ...
            if (FAILED(dev->CreateBuffer(&ib, &id, &res.IndexBuffer))) return false;
            ...
            res.IndexCount = static_cast<uint32_t>(res.SourceIndices.size());
            res.BuildBVH();
