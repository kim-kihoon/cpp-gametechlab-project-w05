...
inline uint32_t Culling8ObjectsAVX2(const float* BaseMinX, const float* BaseMinY, const float* BaseMinZ,
...
UUniformGrid::UUniformGrid(int InW, int InH, int InD, float InCellSize, FSceneDataSOA* InSceneData)
    : Width(InW), Height(InH), Depth(InD), CellSize(InCellSize), SceneData(InSceneData),
      OriginX(-(static_cast<float>(InW) * InCellSize) * 0.5f), OriginY(-(static_cast<float>(InH) * InCellSize) * 0.5f),
      OriginZ(0.0f), TotalEntryCount(0)
{
    ...
    for (int z = 0; z < Depth; ++z)
    {
        for (int y = 0; y < Height; ++y)
        {
            for (int x = 0; x < Width; ++x)
            {
                ...
                Cells[Index].CellBox.Min = {OriginX + (x * CellSize), OriginY + (y * CellSize),
                                            OriginZ + (z * CellSize)};
                Cells[Index].CellBox.Max = {OriginX + ((x + 1) * CellSize), OriginY + ((y + 1) * CellSize),
                                            OriginZ + ((z + 1) * CellSize)};
            }
        }
    }
}
...
void UUniformGrid::BuildGrid()
{
    ...
    {
        const int MinGridX =
            std::clamp(static_cast<int>((SceneData->MinX[ObjectIndex] - OriginX) * InvCellSize), 0, Width - 1);
        const int MinGridY =
            std::clamp(static_cast<int>((SceneData->MinY[ObjectIndex] - OriginY) * InvCellSize), 0, Height - 1);
        const int MinGridZ =
            std::clamp(static_cast<int>((SceneData->MinZ[ObjectIndex] - OriginZ) * InvCellSize), 0, Depth - 1);
        const int MaxGridX =
            std::clamp(static_cast<int>((SceneData->MaxX[ObjectIndex] - OriginX) * InvCellSize), 0, Width - 1);
        const int MaxGridY =
            std::clamp(static_cast<int>((SceneData->MaxY[ObjectIndex] - OriginY) * InvCellSize), 0, Height - 1);
        const int MaxGridZ =
            std::clamp(static_cast<int>((SceneData->MaxZ[ObjectIndex] - OriginZ) * InvCellSize), 0, Depth - 1);
...
void UUniformGrid::QueryFrustum(const Math::FFrustum& Frustum, uint32_t* OutIndices, uint32_t& OutCount,
                                uint32_t MaxCapacity)
{
    OutCount = 0;
    if (!SceneData || !OutIndices || MaxCapacity == 0 || Cells.empty())
        return;
    ...
    for (int z = MinGridZ; z <= MaxGridZ; ++z)
    {
        for (int y = MinGridY; y <= MaxGridY; ++y)
        {
            for (int x = MinGridX; x <= MaxGridX; ++x)
            {
                ...
                if (CellResult == Math::ECullingResult::Outside)
                    continue;
                ...
                if (CellResult == Math::ECullingResult::FullyInside)
                {
                    for (uint32_t i = 0; i < Cell.Count; ++i)
                    {
                        ...
                        if (VisitTokens[Index] == CurrentVisitToken)
                            continue;
                ...
                else
                {
                    for (uint32_t i = 0; i < Cell.Count; ++i)
                    {
                        ...
                        if (Frustum.TestBox(SceneData->MinX[Index], SceneData->MinY[Index], SceneData->MinZ[Index],
                                            SceneData->MaxX[Index], SceneData->MaxY[Index],
...
void UUniformGrid::CullingAndBuildRenderQueue(const Math::FFrustum& Frustum, const Math::FVector& CameraPosVec)
{
    ...
    DirectX::XMStoreFloat3(&cam, CameraPosVec);
            ...
            for (int x = minGX; x <= maxGX; ++x)
            {
                ...
                else
                {
                    for (; i + 8 <= Cell.Count; i += 8)
                    {
                        uint32_t poff[8];
                        uint32_t pcnt =
                            Culling8ObjectsAVX2(SceneData->MinX.data(), SceneData->MinY.data(), SceneData->MinZ.data(),
